  ...
  template:
    ...
    spec:
      ...
      - name: wait-for-cosmos
        image: busybox:1.35
        command: ['sh', '-c', 'until nslookup jd-carenest-cosmos.mongo.cosmos.azure.com; do echo waiting for cosmos; sleep 2; done;']
      containers:
      - name: {name}-service
        image: jdcarenestacr.azurecr.io/{name}:latest
        ...
        - configMapRef:
            name: app-config
        {'- configMapRef:\\n            name: ai-config' if name == 'ai' else ''}
        {'- configMapRef:\\n            name: entra-config' if name == 'auth' else ''}
        volumeMounts:
      ...
      volumes:
        - name: secrets-store-inline
          csi:
            ...
            readOnly: true
            volumeAttributes:
              secretProviderClass: "jd-carenest-kv-secrets"
"""
