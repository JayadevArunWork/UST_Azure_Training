...
function EntraLoginButton({ entraLogin, setError, navigate }) {
  // const { instance } = useMsal();

  const handleEntraLogin = async () => {
    // Entra ID disabled — re-enable on HTTPS
    alert('Microsoft login will be available when HTTPS is configured')
    /*
    try {
      const response = await instance.loginPopup(loginRequest);
      await entraLogin(response.accessToken);
      navigate('/dashboard');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="mt-4">
      <button onClick={handleEntraLogin} disabled className="w-full flex items-center justify-center gap-2 rounded-xl bg-blue-600/50 py-3 font-semibold text-white shadow-lg cursor-not-allowed transition-all">
...
export default function Login() {
  const { login, entraLogin } = useAuth();
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex w-full lg:w-1/2 items-center justify-center p-8 bg-slate-50">
        <div className="w-full max-w-md animate-fade-in-up">
          ...
          <EntraLoginButton entraLogin={entraLogin} setError={setError} navigate={navigate} />
